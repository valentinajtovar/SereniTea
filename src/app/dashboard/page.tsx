  ...
  const fetchJournalEntries = useCallback(async (user: User) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error?.message || 'Failed to fetch entries');
      }
      ...
      toast({ title: "Error", description: (error as Error).message || "No se pudieron cargar las entradas del diario.", variant: "destructive" });
    } finally {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: "No se encontraron los datos del paciente. Por favor, regístrate de nuevo para sincronizar tu cuenta.",
          variant: "destructive",
          duration: 9000,
        });
      } else {
        const errorData = await response.json();
        throw new Error(errorData.error?.message || 'Failed to fetch patient data');
      }
    } catch (error) {
      console.error("Error fetching patient data:", error);
      toast({
        title: "Error",
        description: (error as Error).message || "No se pudieron cargar los datos del paciente.",
        variant: "destructive",
      });
      setPatientData(null);
    }
                ...
                {showFullForm ? (
                  <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                      ...
                      <Button type="submit" size="lg" className="w-full font-bold text-lg" disabled={isAuthLoading}>{isAuthLoading ? 'Cargando...' : 'Guardar Mi Día'}</Button>
                    </form>
                  </Form>
                ) : (
